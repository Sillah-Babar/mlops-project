...
def evaluate_metric(model,new_data):        
    ...
    for i in range(len(new_data[0]['the_input'])):
        ...
        return wer_error/float(count_vids) 

def train_model(new_data,batch_size_,eps,learning_rate,beta1,beta2):
    model=run_experiment(new_data,batch_size_,eps,learning_rate,beta1,beta2)
    return model

def evaluate(ti):
    models=ti.xcom_pull(task_ids=['training_model_A','training_model_B','training_model_C'])
    wer_com=[]
    for i in range(len(models)):
        wer_com.append(evaluate_metric(models[i],new_data))

    accuracy=min(wer_com)
    if(accuracy>1.2):
        return 'accurate'
    else:
        return 'inaccurate'

with DAG("TRAINING_LIPNET",start_date=datetime(2023,6,11),
         schedule=None,catchup=False)as dag:
    
    training_A=PythonOperator(
        task_id="training_model_A",
        python_callable=train_model(new_data,32,2,0.001,0.3,0.2)
    )
    
    training_B=PythonOperator(
        task_id="training_model_B",
        python_callable=train_model(new_data,8,2,0.0001,0.4,0.5)
    )

    training_C=PythonOperator(
        task_id="training_model_C",
        python_callable=train_model(new_data,64,2,0.000001,0.6,0.7)
    )
